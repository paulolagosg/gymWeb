        ...
        <div class="">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4">
                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('planes.update', $plan->slug) }}" method="POST">
                        @csrf
                        @method('PUT')
                        @if($isSuperAdmin)
                        <div class="mb-4 sm:mb-0">
                            ...
                            <select id="id_gimnasio" name="id_gimnasio" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                ...
                                <option value="{{ $gimnasio->id }}" {{ (string) old('id_gimnasio', $plan->id_gimnasio) === (string) $gimnasio->id ? 'selected' : '' }}>
                                    {{ $gimnasio->nombre }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4 sm:mb-0">
                            ...
                            <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $plan->nombre) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            @error('nombre')
                        ...
                        <div class="mb-4 sm:mb-0">
                            ...
                            <textarea id="descripcion" name="descripcion" rows="4" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>{{ old('descripcion', $plan->descripcion) }}</textarea>
                            @error('descripcion')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                            <div class="mb-4 sm:mb-0">
                                ...
                                <input type="number" id="valor" name="valor" value="{{ old('valor', $plan->valor) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                                @error('valor')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="mb-4 sm:mb-0">
                                ...
                                <input type="number" id="porcentaje" name="porcentaje" value="{{ old('porcentaje',$plan->porcentaje) }}" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @error('porcentaje')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
