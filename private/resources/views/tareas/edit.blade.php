    ...
    <div class="py-4">
        <div class="">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <h2 class="text-2xl font-bold p-4">Editar Tarea</h2>
                <div class="p-4 text-gray-900 overflow-x-auto w-full">
                    @if(session('success'))
                    ...
                    @endif
                    <form method="POST" action="{{ route('tareas.update', $tarea->slug) }}" class="mb-4">
                        @csrf
                        @method('PUT')
                        <div class="mt-4">
                        ...
                        <div class="mt-4">
                            ...
                            <input id="nombre" type="text" name="nombre" value="{{ $tarea->nombre }}" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                        </div>
                        <div class="mt-4">
                            <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción</label>
                            <textarea id="descripcion" name="descripcion" rows="3" required
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ $tarea->descripcion }}</textarea>
                        </div>
                        <div class="flex flex-row gap-4">
                            ...
                            <div class="mt-4 flex-1">
                                ...
                                <select id="completada" name="completada" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    <option value="0" {{ $tarea->completada ? '' : 'selected' }}>No</option>
                                    <option value="1" {{ $tarea->completada ? 'selected' : '' }}>Sí</option>
                                </select>
                            </div>
                        </div>
                        <div class="mt-4">
