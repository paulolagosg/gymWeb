<x-admin-layout>
        ...
        <div class="">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    @endif
                    <h2 class="text-lg font-semibold">% de Agua</h2>
                    <div class="mt-4">
                        <p><strong>Fecha de Registro:</strong> {{ $aguaReciente->created_at->format('d/m/Y') }}</p>
                        <p><strong>Actual:</strong> {{ $aguaReciente->valor }} </p>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                        ...
                        <div id="tablaPesos" class="bg-white rounded-lg shadow p-4 w-full">
                            <h3 class="text-md font-semibold">Historial de Agua</h3>
                            @if($aguas->isEmpty())
                            <p>No hay registros de Agua.</p>
                            @else
                            <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap" data-order="{{ \Carbon\Carbon::parse($agua->created_at)->format('Y-m-d') }}">{{ $agua->created_at->format('d/m/Y') }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $agua->valor }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    <form action="{{ route('clientes.aguas.store', $cliente->slug) }}" method="POST" class="mb-6">
                        @csrf
                        <div class="flex flex-col sm:flex-row gap-4 items-end">
                            <div>
                                <label for="peso" class="block text-sm font-medium text-gray-700">Nuevo % de Agua</label>
    ...
    <script id="graficoAguaData" type="application/json">
        @json($aguaChartData)
    </script>
    <script>
        const ctx = document.getElementById('graficoAgua').getContext('2d');
        const aguaChartData = JSON.parse(document.getElementById('graficoAguaData').textContent);
        const labels = aguaChartData.labels ?? [];
        const data = aguaChartData.values ?? [];
