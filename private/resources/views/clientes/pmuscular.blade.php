<x-admin-layout>
        ...
        <div class="">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    @endif
                    <h2 class="text-lg font-semibold">% de Masa Muscular</h2>
                    <div class="mt-4">
                        <p><strong>Fecha de Registro:</strong> {{ $pmuscularReciente->created_at->format('d/m/Y') }}</p>
                        <p><strong>Actual:</strong> {{ $pmuscularReciente->valor }} </p>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                        ...
                        <div class="bg-white rounded-lg shadow p-4 flex items-center justify-center h-72 w-full">
                            <canvas id="graficoPmuscular"></canvas>
                        </div>
                        <!-- Tabla de pesos -->
                        <div id="tablaPesos" class="bg-white rounded-lg shadow p-4 w-full">
                            <h3 class="text-md font-semibold">Historial de % de Masa Muscular</h3>
                            @if($pmuscular->isEmpty())
                            ...
                            <table id="tablaDatos" class="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align: center;">pmuscular</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($pmuscular as $p)
                                    <tr>
                    ...
                    <form action="{{ route('clientes.pmusculares.store', $cliente->slug) }}" method="POST" class="mb-6">
                        @csrf
                        <div class="flex flex-col sm:flex-row gap-4 items-end">
                            <div>
                                <label for="peso" class="block text-sm font-medium text-gray-700">Nuevo % de Masa Muscular</label>
    ...
    <script id="graficoPmuscularData" type="application/json">
        @json($pmuscularChartData)
    </script>
    <script>
        const ctx = document.getElementById('graficoPmuscular').getContext('2d');
        const pmuscularChartData = JSON.parse(document.getElementById('graficoPmuscularData').textContent);
        const labels = pmuscularChartData.labels ?? [];
        const data = pmuscularChartData.values ?? [];
